...
grep -q "TARGET_DEVICES += sl_3000-emmc" "$MK" && sed -i '/TARGET_DEVICES += sl_3000-emmc/d' "$MK"
...
grep -q "TARGET_DEVICES += sl_3000-emmc" "$MK" || { echo "FATAL: MK中未添加TARGET_DEVICES"; exit 1; }
grep -q "CONFIG_TARGET_DEVICE_mediatek_mt7981_DEVICE_sl_3000-emmc=y" "$CFG" || { echo "FATAL: .config中设备未启用"; exit 1; }
# 校验MK设备段唯一
MK_SEG_COUNT=$(grep -c "define Device/sl_3000-emmc" "$MK")
if [ $MK_SEG_COUNT -ne 1 ]; then
  echo "FATAL: MK有$MK_SEG_COUNT个sl_3000-emmc设备段（预期1个）"
  exit 1
fi
...
echo "[OUT] MK  : $MK"
echo "[OUT] CFG : $CFG"
echo "[SUCCESS] 三件套已按指定路径生成，强对齐且符合官方规范！"
